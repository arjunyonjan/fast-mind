    ...
    <div ref={panelRef} style={minimized?{bottom:0,left:"50%",transform:"translateX(-50%)",width:400,height:40}:{right:0,bottom:0,width:size.w,height:size.h}} className="fixed z-[9999] bg-zinc-900/95 backdrop-blur border border-zinc-700 rounded-xl shadow-2xl flex flex-col overflow-hidden text-xs font-mono">
      ...
      {!minimized&&(<>
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1 select-text">
          ...
          {tab==="ai"&&(aiTraces.length===0?<div className="text-zinc-600 text-center py-8">No AI traces yet.</div>:aiTraces.map(t=>(<div key={t.id} className="bg-zinc-800/50 rounded-lg px-3 py-2 border border-zinc-700/50 relative"><button onClick={()=>copyTrace(t)} className="absolute top-2 right-2 p-1 rounded hover:bg-zinc-700 text-zinc-500 hover:text-white">{copiedTraceId===t.id?<Check size={12} className="text-green-400"/>:<Copy size={12}/>}</button><div className="flex items-center justify-between mb-1.5"><span className="text-zinc-500 text-">{t.time}</span><span className={`text- px-1.5 py-0.5 rounded-full ${t.source==="deepseek"?"bg-green-900/40 text-green-300":"bg-zinc-700 text-zinc-400"}`}>{t.source}</span><span className="text-zinc-600 text-">{t.duration}ms</span></div><div className="text-zinc-500 text- mb-1.5 truncate pr-6">Input: <span className="text-zinc-400">"{t.input}"</span></div><div className="grid grid-cols-3 gap-1.5"><div className="bg-zinc-900/50 rounded px-2 py-1"><div className="text-zinc-600 text- uppercase">Title</div><div className="text-zinc-300 text- truncate">{t.parsed.title}</div></div><div className="bg-zinc-900/50 rounded px-2 py-1"><div className="text-zinc-600 text- uppercase">Desc</div><div className="text-zinc-300 text- truncate">{t.parsed.description||"—"}</div></div><div className="bg-zinc-900/50 rounded px-2 py-1"><div className="text-zinc-600 text- uppercase">Priority</div><div className={`text- font-semibold ${priorityColor(t.parsed.priority)}`}>{t.parsed.priority}</div></div></div>{t.steps.length>0&&(<div className="mt-2 pt-2 border-t border-zinc-700/50"><div className="text-zinc-600 text- uppercase mb-1">🧠 Logic Trace</div>{t.steps.map((s,i)=>(<div key={i} className="text-zinc-500 text- flex items-start gap-1"><span className="text-zinc-700">▸</span>{s}</div>))}</div>)}</div>)))}
        </div>
      </>)}
    </div>
  );
