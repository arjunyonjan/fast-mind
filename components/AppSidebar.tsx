...
import { FileText, LayoutGrid, Home, PanelLeftClose, PanelLeft, Zap, Sun, Moon, RefreshCw, FileImage, BarChart3 } from "lucide-react";
...
export default function AppSidebar() {
  ...
  const link = (href: string) =>
    `flex items-center gap-3 px-3 py-2 text-sm rounded-lg transition whitespace-nowrap ${
      pathname === href
        ? "bg-cyan-50 dark:bg-cyan-900/20 text-cyan-600 dark:text-cyan-400 font-medium"
        : "text-zinc-500 dark:text-zinc-400 hover:bg-gray-100 dark:hover:bg-zinc-800 hover:text-zinc-700 dark:hover:text-zinc-200"
    }`;
      ...
      <aside className={`border-r border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 flex flex-col shrink-0 transition-all duration-300 select-text max-sm:fixed max-sm:left-0 max-sm:top-0 max-sm:h-full max-sm:z-50 ${mobileOpen ? "max-sm:w-56" : "max-sm:w-0 max-sm:overflow-hidden"} ${collapsed ? "w-16" : "w-56"}`}>
        ...
        <div className="flex items-center gap-2 px-3 py-4 border-b border-zinc-100 dark:border-zinc-800">
          <Link href="/" className="flex items-center gap-2 shrink-0">
            <div className="h-7 w-7 rounded-lg bg-gradient-to-br from-cyan-500 to-blue-500 flex items-center justify-center"><Zap size={14} className="text-white" /></div>
          </Link>
        ...
        <nav className="flex-1 px-2 py-3 space-y-0.5">
          ...
          <Link href="/token-usage" className={link("/token-usage")} title="Token Usage"><BarChart3 size={17} />{!collapsed && <span>Token Usage</span>}</Link>
        </nav>
        ...
        <div className="px-2 py-3 border-t border-zinc-100 dark:border-zinc-800 space-y-1">
          ...
          {/* Theme toggle */}
          <button onClick={toggle} className="flex items-center gap-3 w-full px-3 py-2 text-sm text-zinc-500 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-lg transition" title={theme === "dark" ? "Switch to light" : "Switch to dark"}>
