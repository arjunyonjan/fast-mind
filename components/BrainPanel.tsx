...
export default function BrainPanel() {
  ...
  const [exerciseLog, setExerciseLog] = useState<{exercise:string,time:string}[]>(() => { try { const s = localStorage.getItem("brain-exercise-log"); return s ? JSON.parse(s) : []; } catch { return []; } });
  const [isAwake, setIsAwake] = useState(true);
  const [brainHealth, setBrainHealth] = useState(100);
  const [initialized, setInitialized] = useState(false);
  const [panelWidth, setPanelWidth] = useState(() => { try { const w = localStorage.getItem("brain-panel-width"); return w ? parseInt(w) : 340; } catch { return 340; } });
  const [sunlight, setSunlight] = useState(() => typeof window !== "undefined" && localStorage.getItem("brain-sunlight") === "true");
  const [social, setSocial] = useState(() => typeof window !== "undefined" && localStorage.getItem("brain-social") === "true");
  const [learning, setLearning] = useState(() => typeof window !== "undefined" && localStorage.getItem("brain-learning") === "true");
  const [breathwork, setBreathwork] = useState(() => typeof window !== "undefined" && localStorage.getItem("brain-breathwork") === "true");
  const [eyeStrain, setEyeStrain] = useState(0);
  const [exerciseLoading, setExerciseLoading] = useState<string | null>(null);
  const simHourRef = useRef<number | null>(null);  const [simHour, setSimHour] = useState<number | null>(null);
  useEffect(() => { simHourRef.current = simHour; }, [simHour]);  const [isLive, setIsLive] = useState(true);
  ...
  useEffect(() => {
    ...
    try { const s = localStorage.getItem("brain-sips"); if (s) { const p = JSON.parse(s); setSips(p); if (p.length) setLastSip(p[p.length-1].time); } } catch {}
    try { const m = localStorage.getItem("brain-mood"); if (m) setMood(m); } catch {}
    try { const l = localStorage.getItem("brain-lifestyle"); if (l) setLifestyle(JSON.parse(l)); } catch {}
  ...
  useEffect(() => {
    const i = setInterval(() => {
      ...
      // Always calculate, even without sip
      const diff = lastSip ? Math.floor((Date.now() - new Date(lastSip).getTime()) / 60000) : 0;
  ...
  return (<>
    ...
    {open && (<div className="fixed inset-0 z-[9999] flex items-start justify-end pointer-events-none"><div style={{ width: panelWidth }} className="pointer-events-auto h-full bg-zinc-900/95 border-l border-zinc-800 shadow-2xl flex flex-col text-xs font-sans text-zinc-300">
      <div className="flex items-center justify-between p-5 pb-2 shrink-0"><h2 className="text-sm font-semibold text-zinc-200 flex items-center gap-2"><Brain size={16} className="text-purple-400" /> YOUR BRAIN</h2><button onClick={() => toggleOpen(false)} className="text-zinc-500 hover:text-white"><X size={14} /></button></div>
      ...
      {!initialized ? (<div className="flex-1 flex items-center justify-center"><div className="text-center space-y-3"><Brain size={40} className="mx-auto text-zinc-600" /><p className="text-zinc-500 text-sm">Your Brain awaits</p><p className="text-zinc-600 text-xs">Log a sip or select a mood to activate.</p></div></div>) : (<div className="flex-1 overflow-y-auto px-5 pb-5 space-y-4">
        ...
        <div className="mt-3 pt-2 border-t border-zinc-800"><div className="flex items-center justify-between mb-1"><span className="text-[9px] text-zinc-500">Time Sim</span><button onClick={() => { const goingLive = isLive; setIsLive(!goingLive); if (goingLive) { setSimHour(6); } else { setSimHour(null); } }} className={`text-[9px] px-2 py-0.5 rounded-full transition ${isLive ? "bg-green-500/10 text-green-400" : "bg-yellow-500/10 text-yellow-400"}`}>{isLive ? "LIVE" : "SIM"}</button></div>{!isLive && <input type="range" min="6" max="29" value={simHour ?? (hour < 6 ? hour + 24 : hour)} onChange={e => setSimHour(parseInt(e.target.value))} className="w-full h-1 accent-cyan-500" />}<div className="flex justify-between text-[8px] text-zinc-600"><span>6a</span><span>12p</span><span>6p</span><span>12a</span><span>6a</span></div></div>
        </div>

        <div className="bg-zinc-800 rounded-xl p-3 flex gap-3"><div className="w-10 h-10 rounded-xl bg-pink-500/20 flex items-center justify-center shrink-0"><Heart size={18} className="text-pink-400 animate-pulse-glow" /></div><div className="flex-1"><div className="text-zinc-500 text-[10px] uppercase mb-2">Mood</div><div className="flex gap-1.5 flex-wrap">{["happy","neutral","sad","anxious","angry","depressed","lonely"].map(m => (<button key={m} onClick={() => saveMood(m)} className={`px-2.5 py-1 rounded-lg text-[10px] capitalize transition ${mood === m ? "bg-cyan-500/20 text-cyan-400" : "bg-zinc-700/50 text-zinc-400 hover:bg-zinc-700"}`}>{m}</button>))}</div></div></div>

        <div className="bg-zinc-800 rounded-xl p-3 flex gap-3"><div className="w-10 h-10 rounded-xl bg-purple-500/20 flex items-center justify-center shrink-0"><Activity size={18} className="text-purple-400" /></div><div className="flex-1"><div className="text-zinc-500 text-[10px] uppercase mb-2">Lifestyle</div><div className="space-y-1">{["overeat","junk food","less sleep","high stress","no exercise","alcohol"].map(item => (<label key={item} className="flex items-center gap-2 text-[10px] text-zinc-400 cursor-pointer"><input type="checkbox" checked={lifestyle.includes(item)} onChange={() => toggleLifestyle(item)} className="w-3 h-3 rounded accent-cyan-500" />{item}</label>))}</div></div></div>

        <div className="bg-zinc-800 rounded-xl p-3"><div className="flex items-center justify-between mb-2"><div className="text-zinc-500 text-[10px] uppercase">Daily Boosters</div><button onClick={() => setInfoPopup("boosters")} className="text-cyan-400 hover:text-cyan-300"><Info size={10} /></button></div><div className="grid grid-cols-2 gap-2">{[{id:"sunlight",icon:<Sun size={16}/>,label:"Sunlight",active:sunlight,action:toggleSunlight,color:"amber"},{id:"social",icon:<Users size={16}/>,label:"Social",active:social,action:toggleSocial,color:"sky"},{id:"learning",icon:<BookOpen size={16}/>,label:"Learning",active:learning,action:toggleLearning,color:"violet"},{id:"breathwork",icon:<Wind size={16}/>,label:"Breathwork",active:breathwork,action:toggleBreathwork,color:"teal"}].map(item=>(<button key={item.id} onClick={item.action} className={`flex items-center gap-1.5 px-2 py-2 rounded-lg text-[10px] transition justify-center ${item.active ? "bg-"+item.color+"-500/20 text-"+item.color+"-400" : "bg-zinc-700/50 text-zinc-500 hover:bg-zinc-700"}`}>{item.icon}{item.label}</button>))}</div></div>

        <div className="bg-zinc-800 rounded-xl p-3"><div className="flex items-center gap-3 mb-3"><div className="w-10 h-10 rounded-xl bg-cyan-500/20 flex items-center justify-center shrink-0"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-cyan-400"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg></div><div className="flex-1"><div className="text-zinc-500 text-[10px] uppercase">Circadian Clock</div><div className="text-lg font-semibold text-cyan-400 tracking-wider">{currentTime}</div><div className="text-[10px] text-zinc-500 mt-0.5">{circadian} PHASE</div><div className="flex items-center gap-2 mt-1.5"><span className="text-[10px] text-zinc-500">Sleeping</span><div onClick={toggleAwake} className={`w-10 h-5 rounded-full relative cursor-pointer transition ${isAwake ? "bg-yellow-500" : "bg-indigo-500"}`}><div className={`w-3.5 h-3.5 rounded-full bg-white absolute top-0.5 transition ${isAwake ? "left-5" : "left-0.5"}`} /></div><span className="text-[10px] text-zinc-500">Awake</span></div></div></div><div className="relative pt-2"><div className="absolute top-0 border-l-[6px] border-r-[6px] border-b-[7px] border-l-transparent border-r-transparent border-b-cyan-400 -translate-x-1/2" style={{ left: ((hour >= 6 ? hour - 6 : hour + 18) / 24 * 100) + "%" }} /><div className="w-full h-3 bg-zinc-700 rounded-full relative overflow-hidden"><div className="absolute inset-0 flex"><div className="w-1/4 bg-amber-500/20 border-r border-zinc-600" /><div className="w-1/4 bg-yellow-400/20 border-r border-zinc-600" /><div className="w-1/4 bg-indigo-500/20 border-r border-zinc-600" /><div className="w-1/4 bg-blue-500/20" /></div></div></div><div className="flex justify-between text-[9px] text-zinc-600 mt-1"><span>6 AM</span><span>NOON</span><span>6 PM</span><span>6 AM</span></div></div>

        <div className="bg-zinc-800 rounded-xl p-3"><div className="flex items-center gap-3"><div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${lastSip ? "bg-cyan-500/20" : "bg-zinc-700"}`}><Droplets size={18} className={lastSip ? "text-cyan-400" : "text-zinc-500"} /></div><div className="flex-1"><div className="text-zinc-500 text-[10px] uppercase">Last Warm Water</div><div className="text-sm font-semibold text-zinc-200">{lastSip ? new Date(lastSip).toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit" }) : "Not logged"}</div><div className="flex items-center gap-1 text-[10px] mt-0.5"><span className={`w-1.5 h-1.5 rounded-full ${minutesSince < 20 ? "bg-green-400" : minutesSince < 45 ? "bg-yellow-400" : "bg-red-400"}`} />{minutesSince} min ago</div></div></div></div>

        <div className="bg-zinc-800 rounded-xl p-3"><div className="flex items-center gap-3"><div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${accumulationPercent > 60 ? "bg-red-500/20" : accumulationPercent > 30 ? "bg-yellow-500/20" : "bg-green-500/20"}`}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={accumulationPercent > 60 ? "text-red-400" : accumulationPercent > 30 ? "text-yellow-400" : "text-green-400"}><path d="M12 2L2 7l10 5 10-5-10-5z"/><path d="M2 17l10 5 10-5"/><path d="M2 12l10 5 10-5"/></svg></div><div className="flex-1"><div className="flex items-center justify-between"><div className="text-zinc-500 text-[10px] uppercase">Waste Accumulation</div><button onClick={() => setInfoPopup("accumulation")} className="text-cyan-400 hover:text-cyan-300"><Info size={10} /></button></div><div className="w-full h-2 bg-zinc-700 rounded-full mt-1.5"><div className={`h-full rounded-full transition-all ${accumulationPercent > 60 ? "bg-red-400" : accumulationPercent > 30 ? "bg-yellow-400" : "bg-green-400"}`} style={{ width: accumulationPercent + "%" }} /></div><div className="text-zinc-500 text-[10px] mt-1"><span className="text-zinc-200 font-semibold">{accumulationPercent}%</span> accumulated</div><div className="text-zinc-500 text-[9px] mt-0.5">tau · amyloid-beta · ROS</div></div></div></div>

        <div className="bg-zinc-800 rounded-xl p-3"><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0 bg-cyan-500/20"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-cyan-400"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg></div><div className="flex-1"><div className="flex items-center justify-between"><div className="text-zinc-500 text-[10px] uppercase">Glymphatic</div><button onClick={() => setInfoPopup("glymphatic")} className="text-cyan-400 hover:text-cyan-300"><Info size={10} /></button></div><div className="w-full h-2 bg-zinc-700 rounded-full mt-1.5"><div className={`h-full rounded-full transition-all ${glymphaticPercent > 60 ? "bg-green-400" : glymphaticPercent > 30 ? "bg-yellow-400" : "bg-red-400"}`} style={{ width: glymphaticPercent + "%" }} /></div><div className="text-zinc-500 text-[10px] mt-1"><span className="text-zinc-200 font-semibold">{glymphaticPercent}%</span> efficient</div><div className="mt-1"><span className="text-[9px] px-1.5 py-0.5 rounded-md bg-green-500/10 text-green-400">Brain waste clearance</span></div></div></div></div>

        <div className="bg-zinc-800 rounded-xl p-3"><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0 bg-cyan-500/20"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-cyan-400"><path d="M12 2a10 10 0 1 0 10 10H12V2z"/><circle cx="12" cy="12" r="3"/></svg></div><div className="flex-1"><div className="flex items-center justify-between"><div className="text-zinc-500 text-[10px] uppercase">Vasodilation</div><button onClick={() => setInfoPopup("vasodilation")} className="text-cyan-400 hover:text-cyan-300"><Info size={10} /></button></div><div className="w-full h-2 bg-zinc-700 rounded-full mt-1.5"><div className={`h-full rounded-full transition-all ${vasodilationPercent > 60 ? "bg-cyan-400" : vasodilationPercent > 30 ? "bg-yellow-400" : "bg-red-400"}`} style={{ width: vasodilationPercent + "%" }} /></div><div className="text-zinc-500 text-[10px] mt-1"><span className="text-zinc-200 font-semibold">{vasodilationPercent}%</span> dilated</div><div className="mt-1"><span className="text-[9px] px-1.5 py-0.5 rounded-md bg-cyan-500/10 text-cyan-400">Brain blood vessel expansion</span></div></div></div></div>

        <div className="bg-zinc-800 rounded-xl p-3"><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl bg-indigo-500/20 flex items-center justify-center shrink-0"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-indigo-400"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg></div><div className="flex-1"><div className="text-zinc-500 text-[10px] uppercase">Sleep Window</div><div className="text-sm font-semibold text-zinc-200">{circadian === "NIGHT" ? "Optimal: Now" : "In ~" + (22 - hour) + " hours"}</div></div></div></div>

        <div className="bg-zinc-800 rounded-xl p-3"><div className="flex items-center justify-between mb-3"><div className="text-zinc-500 text-[10px] uppercase">Neuro Protection</div><button onClick={() => setInfoPopup("exercise")} className="text-cyan-400 hover:text-cyan-300"><Info size={10} /></button></div><div className="grid grid-cols-2 gap-3">{[{id:"sip",icon:<Droplets size={20}/>,label:"Warm Water",color:"purple",action:logSip},{id:"pushups",icon:"🫀",label:"Pushups 50x",color:"emerald",action:()=>logExercise("pushups")},{id:"deadhang",icon:"🧘",label:"Dead Hang 30s",color:"emerald",action:()=>logExercise("deadhang")},{id:"forwardbend",icon:"🙏",label:"Bend 100x",color:"emerald",action:()=>logExercise("forwardbend")}].map(item=>(<button key={item.id} onClick={item.action} className={`w-full aspect-square rounded-2xl max-w-[72px] mx-auto bg-${item.color}-500/10 hover:bg-${item.color}-500/20 flex flex-col items-center justify-center text-${item.color}-400 transition gap-0.5`}>{exerciseLoading === item.id ? <span className="w-5 h-5 border-2 border-current border-t-transparent rounded-full animate-spin" /> : item.icon}<span className="text-[9px] text-zinc-400">{exerciseLoading === item.id ? "Done!" : item.label}</span></button>))}</div></div>

        <div className="text-zinc-600 text-[10px] text-center">{sips.length} sips · {exerciseLog.filter(e => new Date(e.time).toDateString() === new Date().toDateString()).length} exercises today</div>
      </div>)}
    </div></div>)}
