...
export default function BrainPanel() {
  ...
  useEffect(() => {
    const i = setInterval(() => {
      ...
      if (lastSip) {
        ...
        const hoursSince6am = ((hour >= 6 ? hour - 6 : hour + 18) + new Date().getMinutes()/60);
        const naturalAccumulation = Math.min(100, hoursSince6am * 5.5);
        const clearance = glymphaticPercent / 100;
        setAccumulationPercent(Math.round(naturalAccumulation * (1 - clearance * 0.7)));
  ...
  const infoData: Record<string, { title: string; body: string }> = {
    ...
    circadian: { title: "Circadian Rhythm", body: "Your brain's 24-hour clock regulates sleep, focus, and hormone release." }, accumulation: { title: "Waste Accumulation", body: "During waking hours, metabolic waste builds up: tau (~0.5ng/mL/hr), amyloid-beta (~2ng/mL/hr), and ROS. Starts at 6 AM, increases ~5.5% per hour. Sleep and hydration activate glymphatic clearance. High levels impair cognition." }
  };
  ...
  return (<>
    ...
    {infoPopup && (<div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setInfoPopup(null)}><div className="bg-zinc-900 border border-zinc-700 rounded-2xl p-6 w-full max-w-2xl text-sm font-sans text-zinc-300" onClick={e => e.stopPropagation()}><div className="flex items-center justify-between mb-3"><h3 className="text-sm font-semibold text-cyan-400">{infoData[infoPopup].title}</h3><button onClick={() => setInfoPopup(null)} className="text-zinc-500 hover:text-white"><X size={14} /></button></div><div className="w-full h-56 bg-zinc-800 rounded-xl mb-3 flex items-center justify-center text-zinc-600 text-[10px]">🧠 Image placeholder</div><p className="text-zinc-400 leading-relaxed">{infoData[infoPopup].body}</p><button onClick={() => setInfoPopup(null)} className="mt-4 w-full py-3 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 rounded-lg text-sm transition">Got it</button></div></div>)}
  </>);
}