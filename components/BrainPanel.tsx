...
import { Brain, Droplets, X, Info, Heart, Activity, Sun, Users, BookOpen, Wind, ChevronDown, Check } from "lucide-react";
...
import { calcDigestionPhase, calcEnergyLevel } from "@/features/brain/domain/digestion";
...
export default function BrainPanel() {
  ...
  const logMeal = () => { setExerciseLoading("meal"); setTimeout(() => { setLastMeal(new Date().toISOString()); localStorage.setItem("brain-last-meal", new Date().toISOString()); setExerciseLoading(null); setInitialized(true); }, 500); };
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Migrate lifestyle from lowercase to Title Case
      const l = localStorage.getItem("brain-lifestyle");
      if (l) {
        const oldItems = JSON.parse(l);
        const map: Record<string, string> = {
          "overeat":"Overeat","junk food":"Processed / Junk Food","junk":"Processed / Junk Food",
          "processed food":"Processed / Junk Food","processed":"Processed / Junk Food",
          "high sugar":"High Sugar",
          "caffeine":"Caffeine","alcohol":"Alcohol","smoking":"Smoking","less sleep":"Less Sleep",
  ...
  }, [lastSip, lastMeal, notified, mood, lifestyle, isAwake, sunlight, social, learning, breathwork, accumulationPercent, glymphaticPercent, simHour, isLive]);
  ...
  // Digestion calculations
  const hoursSinceMeal = lastMeal ? (Date.now() - new Date(lastMeal).getTime()) / 3600000 : 12;
  const digestionPhase = Math.round(calcDigestionPhase(hoursSinceMeal));
  const energyLevel = Math.round(calcEnergyLevel(hour, isAwake, hoursSinceMeal));
  const energyColor = energyLevel > 60 ? "green" : energyLevel > 30 ? "yellow" : "red";

  const onResize = (e: React.MouseEvent) => {
    e.preventDefault();
    const sx = e.clientX; const sw = panelWidthRef.current;
    const mv = (ev: MouseEvent) => setPanelWidth(Math.max(280, sw + sx - ev.clientX));
    const up = () => { localStorage.setItem("brain-panel-width", String(panelWidthRef.current)); window.removeEventListener("mousemove", mv); window.removeEventListener("mouseup", up); };
    window.addEventListener("mousemove", mv); window.addEventListener("mouseup", up);
  };

  // Shared content component — adapts to fullscreen grid
  const PanelContent = (
    <>
      {/* Row 1: Header stats (always full-width in panel, spans all cols in fullscreen) */}
      <div className={isFullscreen ? 'md:col-span-full' : ''}>
        <div className="sticky top-0 bg-zinc-900/95 backdrop-blur z-10 -mx-5 sm:-mx-6 px-5 sm:px-6 pt-2 pb-3">
          <div className={`grid gap-3 sm:gap-5 ${isFullscreen ? 'grid-cols-2 sm:grid-cols-3 lg:grid-cols-6' : 'grid-cols-1 sm:grid-cols-3'} max-w-2xl mx-auto`}>
            <CircularProgress value={brainHealth} size={96} strokeWidth={6} color={hc} label="Brain" />
            <CircularProgress value={Math.max(0, 100 - eyeStrain * 10)} size={96} strokeWidth={6} color={ec} label="Eyes" />
            <CircularProgress value={75} size={96} strokeWidth={6} color="cyan" label="Spine" />
            <CircularProgress value={digestionPhase} size={96} strokeWidth={6} color="orange" label="Digestion" />
            <CircularProgress value={energyLevel} size={96} strokeWidth={6} color={energyColor} label="Energy" />
            <CircularProgress value={Math.min(100, (glymphaticPercent + vasodilationPercent) / 2)} size={96} strokeWidth={6} color="cyan" label="Oxygen" />
          </div>
      ...
      <CollapsibleCard id="mood" title="Mood" icon={<Heart size={18} className="text-pink-400 animate-pulse-glow" />} iconBg="bg-pink-500/20" collapsed={collapsedCards.includes("mood")} onToggle={() => toggleCard("mood")}>
        <div className="flex gap-1.5 flex-wrap">{MOODS.map(m => (<button key={m.id} onClick={() => saveMood(m.id)} className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[13px] capitalize transition ${mood === m.id ? `${m.activeBg} ${m.activeText} ring-1 ring-white/10` : "bg-zinc-700/50 text-zinc-400 hover:bg-zinc-700"}`}><span className="text-base leading-none">{m.emoji}</span>{m.id}</button>))}</div>
      </CollapsibleCard>
      ...
      <CollapsibleCard id="lifestyle" title="Lifestyle" icon={<Activity size={18} className="text-purple-400" />} iconBg="bg-purple-500/20" collapsed={collapsedCards.includes("lifestyle")} onToggle={() => toggleCard("lifestyle")}>
        <div className="grid grid-cols-2 gap-1.5">{["Overeat","Processed / Junk Food","High Sugar","Caffeine","Alcohol","Smoking","Less Sleep","High Stress","No Exercise","Sitting Too Long","Poor Posture","Overworking","Endless Social Media"].map(item => {
          const on = lifestyle.includes(item);
          return (
            <button key={item} onClick={() => toggleLifestyle(item)} className={`flex items-center gap-2 px-2.5 py-2 rounded-lg text-[12px] transition border ${on ? "bg-cyan-500/10 border-cyan-500/30 text-cyan-400" : "bg-zinc-700/30 border-zinc-700/50 text-zinc-500 hover:border-zinc-600"}`}>
              <div className={`w-3.5 h-3.5 rounded flex items-center justify-center shrink-0 transition ${on ? "bg-cyan-500" : "border border-zinc-600"}`}>
        ...
        <div className="grid grid-cols-2 gap-1.5">
          {[
            ...
            <button key={item.id} onClick={item.action} className={`flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-[12px] font-medium transition border ${item.active ? `${item.bg} ${item.border} ${item.text}` : "bg-zinc-700/30 border-zinc-700/50 text-zinc-500 hover:border-zinc-600 hover:text-zinc-300"}`}>
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 transition ${item.active ? item.iconBg : "bg-zinc-700/50"}`}>
                {item.icon}
              </div>
      ...
      <CollapsibleCard id="neuro" title="Neuro Protection" icon={null} iconBg="" collapsed={collapsedCards.includes("neuro")} onToggle={() => toggleCard("neuro")} infoAction={() => setInfoPopup("exercise")}>
        <div className="grid grid-cols-4 sm:grid-cols-5 gap-2 max-w-sm mx-auto">
          {[
            { id: "sip", icon: <Droplets size={18} />, label: "Water", color: "purple", bg: "bg-purple-500/10", hover: "hover:bg-purple-500/20", text: "text-purple-400", action: logSip },
            { id: "meal", icon: "🍽️", label: "Meal", color: "orange", bg: "bg-orange-500/10", hover: "hover:bg-orange-500/20", text: "text-orange-400", action: logMeal },
            { id: "pushups", icon: "🫀", label: "Pushups", color: "emerald", bg: "bg-emerald-500/10", hover: "hover:bg-emerald-500/20", text: "text-emerald-400", action: () => logExercise("pushups") },
            { id: "deadhang", icon: "🧘", label: "Hang", color: "emerald", bg: "bg-emerald-500/10", hover: "hover:bg-emerald-500/20", text: "text-emerald-400", action: () => logExercise("deadhang") },
            { id: "forwardbend", icon: "🙏", label: "Bend", color: "emerald", bg: "bg-emerald-500/10", hover: "hover:bg-emerald-500/20", text: "text-emerald-400", action: () => logExercise("forwardbend") },
          ].map(item => (
            <button key={item.id} onClick={item.action} className={`w-full aspect-square rounded-2xl ${item.bg} ${item.hover} ${item.text} flex flex-col items-center justify-center transition gap-1`}>
              {exerciseLoading === item.id ? <span className="w-5 h-5 border-2 border-current border-t-transparent rounded-full animate-spin" /> : <span className="text-lg">{item.icon}</span>}
              <span className="text-[10px] text-zinc-400">{exerciseLoading === item.id ? "Done!" : item.label}</span>
            </button>
          ))}
        </div>
      </CollapsibleCard>
  ...
  return (<>
    ...
    {open && (
      <>
        ...
        <div
          className={`fixed z-[9999] flex flex-col bg-zinc-900 text-base font-sans text-zinc-300 transition-all duration-300
            ${isFullscreen
              ? 'inset-0 sm:rounded-none'
              : 'inset-0 sm:inset-y-0 sm:right-0 sm:left-auto sm:shadow-2xl sm:border-l sm:border-zinc-800'
            }
            animate-slide-in
          `}
          style={{
            // On desktop, respect resizable width when not fullscreen
            ...(typeof window !== 'undefined' && window.innerWidth >= 640 && !isFullscreen ? { width: panelWidth } : {}),
          }}
        >
          {/* Header */}
          <div className="flex items-center justify-between p-4 sm:p-5 pb-3 shrink-0 border-b border-zinc-800">
            <h2 className="text-sm font-semibold text-zinc-200 flex items-center gap-2">
              ...
              <span className="hidden sm:inline">YOUR BRAIN</span>
              <span className="text-[9px] font-normal text-zinc-500 sm:ml-1">powered by</span>
              <span className="text-[9px] font-semibold bg-gradient-to-r from-amber-400 to-orange-500 bg-clip-text text-transparent">Buddha Engine</span>
            </h2>
            <div className="flex items-center gap-1">
              {!isFullscreen && (
                <button onClick={toggleFullscreen} className="p-1.5 rounded text-zinc-500 hover:text-white sm:inline hidden transition" title="Expand">
              ...
              {isFullscreen && (
                <button onClick={toggleFullscreen} className="p-1.5 rounded text-zinc-500 hover:text-white sm:inline hidden transition" title="Shrink">
                  <svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                    <polyline points="1,6 1,1 6,1"/><polyline points="15,10 15,15 10,15"/><line x1="1" y1="1" x2="7" y2="7"/><line x1="15" y1="15" x2="9" y2="9"/>
                  </svg>
                </button>
              )}
              <button onClick={() => toggleOpen(false)} className="p-1.5 rounded text-zinc-500 hover:text-white"><X size={18} /></button>
            </div>
          </div>
          ...
          {!initialized ? (
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center space-y-3">
                <Brain size={40} className="mx-auto text-zinc-600" />
                <p className="text-zinc-500 text-sm">Your Brain awaits</p>
                <p className="text-zinc-600 text-xs">Log a sip or select a mood to activate.</p>
              </div>
            </div>
          ) : isFullscreen ? (
            /* ===== FULLSCREEN: Hero + 3-col grid ===== */
            <div className="flex-1 overflow-y-auto">
              ...
              <div className="bg-gradient-to-b from-zinc-800/50 to-transparent border-b border-zinc-800 px-6 sm:px-10 py-8">
                <div className="max-w-6xl mx-auto">
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 sm:gap-6 mb-4">
                    <CircularProgress value={brainHealth} size={96} strokeWidth={6} color={hc} label="Brain" />
                    <CircularProgress value={Math.max(0, 100 - eyeStrain * 10)} size={96} strokeWidth={6} color={ec} label="Eyes" />
                    <CircularProgress value={75} size={96} strokeWidth={6} color="cyan" label="Spine" />
                    <CircularProgress value={digestionPhase} size={96} strokeWidth={6} color="orange" label="Digestion" />
                  ...
                  <div className="flex items-center justify-center gap-3 text-[10px]">
                    <span className={`px-2.5 py-1 rounded-full ${circadian === "NIGHT" && !isAwake ? "bg-green-500/10 text-green-400" : eyeStrain >= 7 ? "bg-red-500/10 text-red-400" : eyeStrain >= 4 ? "bg-yellow-500/10 text-yellow-400" : "bg-zinc-700 text-zinc-500"}`}>{circadian === "NIGHT" && !isAwake ? "Eyes recovering" : eyeStrain >= 7 ? "High eye strain" : eyeStrain >= 4 ? "Moderate strain" : "Eyes rested"}</span>
                    {circadian === "NIGHT" && !isAwake && <span className="px-2.5 py-1 rounded-full bg-green-500/10 text-green-400">CSF cleaning active</span>}
                    {circadian === "NIGHT" && isAwake && <span className="px-2.5 py-1 rounded-full bg-red-500/10 text-red-400">CSF cleaning blocked</span>}
                  </div>
                  {/* Time Sim */}
                  <div className="mt-4 pt-4 border-t border-zinc-800 max-w-xs mx-auto">
                    <div className="flex items-center justify-between mb-2"><span className="text-[9px] text-zinc-500">Time Simulation</span>
                      <div className="flex items-center gap-2">
                        <span className="text-[9px] text-zinc-500">SIM</span>
                        <div onClick={() => { const goingLive = isLive; setIsLive(!goingLive); if (goingLive) { setSimHour(6); } else { setSimHour(null); } }} className={`w-10 h-5 rounded-full relative cursor-pointer transition ${isLive ? "bg-green-500" : "bg-yellow-500"}`}><div className={`w-3.5 h-3.5 rounded-full bg-white absolute top-0.5 transition ${isLive ? "left-5" : "left-0.5"}`} /></div>
                        <span className="text-[9px] text-zinc-500">LIVE</span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-w-6xl mx-auto px-6 sm:px-10 py-6">
                {/* Circadian Clock — spans full width */}
                ...
                <CollapsibleCard id="neuro" title="Neuro Protection" icon={null} iconBg="" collapsed={collapsedCards.includes("neuro")} onToggle={() => toggleCard("neuro")} infoAction={() => setInfoPopup("exercise")}>
                  <div className="grid grid-cols-4 sm:grid-cols-5 gap-2 max-w-sm mx-auto">
                    {[
                      { id: "sip", icon: <Droplets size={18} />, label: "Water", bg: "bg-purple-500/10", hover: "hover:bg-purple-500/20", text: "text-purple-400", action: logSip },
                      { id: "meal", icon: "🍽️", label: "Meal", bg: "bg-orange-500/10", hover: "hover:bg-orange-500/20", text: "text-orange-400", action: logMeal },
                      { id: "pushups", icon: "🫀", label: "Pushups", bg: "bg-emerald-500/10", hover: "hover:bg-emerald-500/20", text: "text-emerald-400", action: () => logExercise("pushups") },
                      { id: "deadhang", icon: "🧘", label: "Hang", bg: "bg-emerald-500/10", hover: "hover:bg-emerald-500/20", text: "text-emerald-400", action: () => logExercise("deadhang") },
                      { id: "forwardbend", icon: "🙏", label: "Bend", bg: "bg-emerald-500/10", hover: "hover:bg-emerald-500/20", text: "text-emerald-400", action: () => logExercise("forwardbend") },
                    ].map(item => (
                      <button key={item.id} onClick={item.action} className={`w-full aspect-square rounded-2xl ${item.bg} ${item.hover} ${item.text} flex flex-col items-center justify-center transition gap-1`}>
                        {exerciseLoading === item.id ? <span className="w-5 h-5 border-2 border-current border-t-transparent rounded-full animate-spin" /> : <span className="text-lg">{item.icon}</span>}
                        <span className="text-[10px] text-zinc-400">{exerciseLoading === item.id ? "Done!" : item.label}</span>
                ...
                <div className="md:col-span-full text-zinc-600 text-[12px] text-center">{sips.length} sips · {exerciseLog.filter(e => new Date(e.time).toDateString() === new Date().toDateString()).length} exercises today</div>
              </div>
            </div>
          ) : (
            /* ===== PANEL MODE: Single column stack ===== */
        ...
        {/* Backdrop (hidden when fullscreen on desktop) */}
        {!isFullscreen && <div className="fixed inset-0 z-[9998] bg-black/50 sm:bg-black/30" onClick={() => toggleOpen(false)} />}

        {/* Resize handle — sits between backdrop and panel in z-order, outside panel */}
        {!isFullscreen && (
          <div
