...
export default function DocumentsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      const url = search 
        ? `/api/documents?search=${encodeURIComponent(search)}&page=1&limit=20`
      ...
      if (data.success) {
        ...
        setTotal(data.total);
        setHasMore(data.documents.length === 20 && data.documents.length < data.total);
        if (search) setPage(1);
      }
      setLoading(false);
    };
  ...
  const refresh = () => {
    setLoading(true);
    fetch(`/api/documents?page=${page}&limit=20`).then(r => r.json()).then(data => {
      if (data.success) { setDocs(data.documents); setTotal(data.total); setHasMore(page * 20 < data.total); }
  ...
  if (loading && docs.length === 0) {
    return <div className="fixed top-0 left-0 right-0 h-0.5 z-50"><div className="h-full bg-gradient-to-r from-cyan-500 to-blue-500 animate-loading-bar" /></div>;
  }
      ...
      <div className="border-b border-zinc-100 dark:border-zinc-800 px-6 py-4 shrink-0">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-400 dark:text-zinc-500">{total} documents</p>
            </div>
          </div>
        ...
        <div className="flex items-center gap-3 mt-3">
          <div className="relative flex-1 max-w-md">
            ...
            />
            {searchLoading && <div className="absolute right-3 top-1/2 -translate-y-1/2"><div className="w-4 h-4 border-2 border-emerald-500 border-t-transparent rounded-full animate-spin" /></div>}
          </div>
          <select
            value={sortBy}
      ...
      <div className="flex-1 min-h-0 overflow-y-auto">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            {!search && <Link href="/documents/new" className="mt-2 text-emerald-500 hover:text-emerald-400 text-sm font-medium">Create one →</Link>}
          </div>
        ) : view === "list" ? (
          <div className="max-w-5xl mx-auto px-6 py-4">
            ...
            {Object.entries(groups).map(([key, group]) => group.length > 0 && (
              <div key={key}>
                    ...
                    <div className="flex items-center gap-4 px-3 py-2.5">
                      <Link href={`/documents/${doc._id}`} className="flex-1 min-w-0 flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          ...
                          {doc.content && <span className="text-xs text-zinc-400 dark:text-zinc-500 truncate block">{stripHtml(doc.content)}</span>}
                        </div>
                      </Link>
                      ...
                      <div className="w-8 flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => setDeleteId(doc._id)} className="p-1 rounded hover:bg-zinc-200 dark:hover:bg-zinc-700 text-zinc-400 hover:text-red-500 transition">
                          <Trash2 size={12} />
                        </button>
                      </div>
                    </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {filtered.map(doc => (
                <Link key={doc._id} href={`/documents/${doc._id}`} className="group bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-4 hover:shadow-md hover:border-emerald-200 dark:hover:border-emerald-800 hover:-translate-y-0.5 transition-all duration-200 relative">
                  ...
                  <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); setDeleteId(doc._id); }} className="absolute top-2 right-2 p-1 rounded opacity-0 group-hover:opacity-100 hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-400 hover:text-red-500 transition">
                    <X size={12} />
                  </button>
                </Link>
              ))}
