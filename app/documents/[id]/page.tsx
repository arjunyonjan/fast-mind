...
export default function ViewDocumentPage() {
  ...
  useEffect(() => {
    fetch(`/api/documents/${id}`).then(res => res.json()).then(data => { if (data.success) setDoc(data.document); setLoading(false); });
  }, [id]);

  async function handleDelete() {
    setShowModal(false);
    const res = await fetch(`/api/documents/${id}`, { method: 'DELETE' });
    if ((await res.json()).success) { showToast('Document deleted!', 'success'); router.push('/'); }
  ...
  return (
    <>
      <ConfirmModal isOpen={showModal} title="Delete Document" message={`Delete "${doc.title}"? This cannot be undone.`} onConfirm={handleDelete} onCancel={() => setShowModal(false)} />
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 mt-4 overflow-hidden">
          <div className="p-6 border-b border-gray-100">
            ...
            <div className="flex gap-3 mt-4">
              <Link href={`/documents/${id}/edit`} className="px-4 py-1.5 bg-cyan-500 text-white rounded-lg hover:bg-cyan-600">Edit</Link>
              <button onClick={() => setShowModal(true)} className="px-4 py-1.5 border border-red-300 text-red-600 rounded-lg hover:bg-red-50">Delete</button>
            </div>
          </div>
