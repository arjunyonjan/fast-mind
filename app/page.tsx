...
export default function Home() {
  ...
  const handleSend = async () => {
    ...
    setInput('');
    setAiLoading(true);
    // TODO: Replace with your actual AI endpoint (Hugging Face, OpenAI, etc.)
    const res = await fetch("/api/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message: userMsg.content }) }); const data = await res.json(); const assistantMsg = { id: (Date.now()+1).toString(), role: "assistant", content: data.reply || "Done." }; setMessages(prev => [...prev, assistantMsg]); setAiLoading(false);
  };
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-cyan-50/30">
        ...
        <div className="border-b bg-white/70 backdrop-blur-sm sticky top-0 z-10">
          <div className="max-w-5xl mx-auto px-4 py-3 flex justify-between items-center">
            ...
            {taskCount > 0 && <span className="bg-red-500 text-white text-xs rounded-full px-1.5 py-0.5">{taskCount}</span>}</Link>
            </div>
          </div>
        </div>

        {/* Main content: centered chat input + recent documents */}
          ...
          <div className="max-w-3xl mx-auto">
            <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
              <textarea
                ...
                className="w-full p-5 text-gray-700 border-none resize-none focus:outline-none text-base"
                rows={3}
              />
            ...
            {documents.length === 0 ? (
              ...
              documents.map((doc) => (
                <Link key={doc._id} href={`/documents/${doc._id}`} className="block bg-white border border-gray-100 rounded-xl p-5 hover:shadow-md hover:border-cyan-200 transition group">
                  ...
                  <p className="text-sm text-gray-400 mt-1">Updated {new Date(doc.updatedAt).toLocaleDateString()}</p>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-cyan-50/30 flex flex-col">
      ...
      <div className="border-b bg-white/70 backdrop-blur-sm sticky top-0 z-10">
        <div className="max-w-4xl mx-auto px-4 py-3 flex justify-between items-center">
          ...
          <div className="flex gap-4 text-sm">
            <button onClick={() => setMessages([])} className="text-gray-500 hover:text-gray-700">New chat</button>`n            <Link href="/tasks" className="flex items-center gap-1 text-gray-600 hover:text-cyan-600"><LayoutGrid className="w-4 h-4" /> Tasks
            {taskCount > 0 && <span className="bg-red-500 text-white text-xs rounded-full px-1.5 py-0.5">{taskCount}</span>}
            {taskCount > 0 && <span className="bg-red-500 text-white text-xs rounded-full px-1.5 py-0.5">{taskCount}</span>}</Link>
            <Link href="/documents" className="flex items-center gap-1 text-gray-600 hover:text-cyan-600"><FileText className="w-4 h-4" /> Docs</Link>
            <Link href="/tasks" className="flex items-center gap-1 text-gray-600 hover:text-cyan-600"><LayoutGrid className="w-4 h-4" /> Tasks
      ...
      <div className="border-t bg-white/80 backdrop-blur-sm p-4">
        <div className="max-w-3xl mx-auto flex gap-3">
          <textarea
            ...
            className="flex-1 p-3 border rounded-xl resize-none focus:outline-none focus:ring-2 focus:ring-cyan-500"
            rows={1}
