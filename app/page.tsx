  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-12">
      <div className="text-center mb-12">
        ...
        <p className="text-gray-500 mt-2">Lightning-fast document writing</p>
        <Link href="/documents/new" className="inline-block mt-6 bg-gradient-to-r from-cyan-500 to-blue-500 text-white px-6 py-2.5 rounded-lg font-medium hover:shadow-lg transition">
          + New Document
        </Link>
      </div>
        ...
        {documents.length === 0 ? (
          ...
          documents.map((doc) => (
            <Link key={doc._id} href={`/documents/${doc._id}`} className="block bg-white border border-gray-100 rounded-xl p-5 hover:shadow-md hover:border-cyan-200 transition group">
              ...
              {doc.content && (
                <p className="text-sm text-gray-500 mt-2 line-clamp-2">{doc.content.replace(/<[^>]*>/g, '').substring(0, 150)}...</p>
              )}
            </Link>
          ))
        )}
