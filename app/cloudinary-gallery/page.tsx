...
export default function CloudinaryGalleryPage() {
  ...
  useEffect(() => {
    ...
    async function fetchImages() {
      try {
        ...
        if (!cancelled) setError("Unable to reach Cloudinary");
      } finally {
    ...
    return (
      <div className="bg-zinc-50 dark:bg-zinc-950 flex items-center justify-center p-6 h-full">
        <div className="flex flex-col items-center gap-3 text-zinc-400">
          ...
          <p className="text-sm text-zinc-400 dark:text-zinc-500">Upload images to Cloudinary to see them here.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-zinc-50 dark:bg-zinc-950 p-6 sm:p-10 h-full overflow-y-auto">
      <div className="max-w-6xl mx-auto">
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-zinc-800 dark:text-zinc-100">Cloudinary Gallery</h1>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
          {images.map((img) => (
            ...
            >
              <img
                src={img.url.replace("/upload/", "/upload/c_fill,w_200,h_200/")}
